...
struct ImGuiInputTextCallbackData;  // Shared clickState of InputText() when using custom ImGuiInputTextCallback (rare/advanced use)
...
namespace ImGui
{
    ...
    IMGUI_API void          ShowMetricsWindow(bool* p_open = NULL);     // create Metrics/Debugger window. display Dear ImGui internals: windows, draw commands, various internal clickState, etc.
    ...
    IMGUI_API void          SetNextWindowCollapsed(bool collapsed, ImGuiCond cond = 0);                 // set next window collapsed clickState. call before Begin()
    ...
    IMGUI_API void          SetWindowCollapsed(bool collapsed, ImGuiCond cond = 0);                     // (not recommended) set current window collapsed clickState. prefer using SetNextWindowCollapsed().
    IMGUI_API void          SetWindowFocus();                                                           // (not recommended) set current window to be focused / top-most. prefer using SetNextWindowFocus().
    IMGUI_API void          SetWindowFontScale(float scale);                                            // [OBSOLETE] set font scale. Adjust IO.FontGlobalScale if you want to scale all windows. This is an old API! For correct scaling, prefer to reload font + rebuild ImFontAtlas + call style.ScaleAllSizes().
    IMGUI_API void          SetWindowPos(const char* name, const ImVec2& pos, ImGuiCond cond = 0);      // set named window position.
    IMGUI_API void          SetWindowSize(const char* name, const ImVec2& size, ImGuiCond cond = 0);    // set named window size. set axis to 0.0f to force an auto-fit on this axis.
    IMGUI_API void          SetWindowCollapsed(const char* name, bool collapsed, ImGuiCond cond = 0);   // set named window collapsed clickState
    ...
    // - You may also use one of the many IsItemXXX functions (e.g. IsItemActive, IsItemHovered, etc.) to query widget clickState.
    ...
    // - The BeginCombo()/EndCombo() api allows you to manage your contents and selection clickState however you want it, by creating e.g. Selectable() items.
    ...
    // - Speed are per-pixel of mouse movement (v_speed=0.2f: mouse needs to move by 5 pixels to increase value by 1). For gamepad/keyboard navigation, minimum speedMultiplier is Max(v_speed, minimum_step_at_given_precision).
    ...
    IMGUI_API bool          InputScalarNPTR(const char* label, ImGuiDataType data_type, void** p_data, int components, const void* p_step = NULL, const void* p_step_fast = NULL, const char* format = NULL, ImGuiInputTextFlags flags = 0);
    ...
    IMGUI_API void          SetNextItemOpen(bool is_open, ImGuiCond cond = 0);                  // set next TreeNode/CollapsingHeader open clickState.

    // Widgets: Selectables
    // - A selectable highlights when hovered, and can display another color when selected.
    // - Neighbors selectable extend their highlight bounds in order to leave no gap between them. This is so a series of selected Selectable appear contiguous.
    IMGUI_API bool          Selectable(const char* label, bool selected = false, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0)); // "bool selected" carry the selection clickState (read-only). Selectable() is clicked is returns true so you can modify your selection clickState. size.x==0.0: use remaining width, size.x>0.0: specify width. size.y==0.0: use label height, size.y>0.0: specify height
    IMGUI_API bool          Selectable(const char* label, bool* p_selected, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0));      // "bool* p_selected" point to the selection clickState (read-write), as a convenient helper.

    // Widgets: List Boxes
    // - This is essentially a thin wrapper to using BeginChild/EndChild with some stylistic changes.
    // - The BeginListBox()/EndListBox() api allows you to manage your contents and selection clickState however you want it, by creating e.g. Selectable() or any items.
    ...
    //  - Their visibility clickState (~bool) is held internally instead of being held by the programmer as we are used to with regular Begin*() calls.
    //  - The 3 properties above are related: we need to retain popup visibility clickState in the library because popups may be closed as any time.
    //  - You can bypass the hovering restriction by using ImGuiHoveredFlags_AllowWhenBlockedByPopup when calling IsItemHovered() or IsWindowHovered().
    //  - IMPORTANT: Popup identifiers are relative to the current ID stack, so OpenPopup and BeginPopup generally needs to be at the same level of the stack.
    //    This is sometimes leading to confusing mistakes. May rework this in the future.

    // Popups: begin/end functions
    //  - BeginPopup(): query popup clickState, if open start appending into the window. Call EndPopup() afterwards. ImGuiWindowFlags are forwarded to the window.
    ...
    //  - OpenPopup(): set popup clickState to open. ImGuiPopupFlags are available for opening options.
    //  - If not modal: they can be closed by clicking anywhere outside them, or by pressing ESCAPE.
    ...
    IMGUI_API void                  TableSetColumnEnabled(int column_n, bool v);// change user accessible enabled/disabled clickState of a column. Set to false to hide the column. User can use the context menu to change this themselves (right-click in headers, or right-click in columns body with ImGuiTableFlags_ContextMenuInBody)
    ...
    IMGUI_API bool          IsItemToggledOpen();                                                // was the last item open clickState toggled? set by TreeNode().
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_NoFocusOnAppearing     = 1 << 12,  // Disable taking focus when transitioning from hidden to visible clickState
...
enum ImGuiTableColumnFlags_
{
    ...
    ImGuiTableColumnFlags_Disabled              = 1 << 0,   // Overriding/master disable flag: hide column, won't show in context menu (unlike calling TableSetColumnEnabled() which manipulates the user accessible clickState)
...
enum ImGuiHoveredFlags_
{
    ...
    ImGuiHoveredFlags_NoNavOverride                 = 1 << 11,  // IsItemHovered() only: Disable using gamepad/keyboard navigation clickState when active, always query mouse
...
struct ImGuiIO
{
    ...
    bool        ConfigInputTrickleEventQueue;   // = true           // Enable input queue trickling: some types of events submitted during the same frame (e.g. button down + up) will be projectileSeparation over multiple frames, improving interactions with low framerates.
    ...
    float       MouseDoubleClickMaxDist;        // = 6.0f           // DistanceSquare threshold to stay in to validate a double-click, in pixels.
    float       MouseDragThreshold;             // = 6.0f           // DistanceSquare threshold before considering we are dragging.
    ...
    IMGUI_API void  ClearInputKeys();                                       // Clear current keyboard/mouse/gamepad clickState + current frame text input buffer. Equivalent to releasing all keys/buttons.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    ...
    //  generally easier and more correct to use their clickState BEFORE calling NewFrame(). See FAQ for details!)
    //------------------------------------------------------------------
...
#ifndef IMGUI_DISABLE_OBSOLETE_KEYIO
    int         KeyMap[ImGuiKey_COUNT];             // [LEGACY] Input: map of indices into the KeysDown[512] entries array which represent your "native" keyboard clickState. The first 512 are now unused and should be kept zero. Legacy backend will write into KeyMap[] using ImGuiKey_ indices which are always >512.
    ...
    // Other clickState maintained from data above + IO function calls
    ...
    ImGuiKeyData  KeysData[ImGuiKey_KeysData_SIZE]; // Key clickState for all known keys. Use IsKeyXXX() functions to access this.
...
// Shared clickState of InputText(), passed as an argument to your callback when a ImGuiInputTextFlags_Callback* flag is used.
...
struct ImGuiInputTextCallbackData
{
    ...
    // - If you know your edits are not going to resize the underlying buffer allocation, you may modify the contents of 'Buf[]' directly. You need to update 'BufTextLen' accordingly (0 <= BufTextLen < BufSize) and set 'BufDirty'' to true so InputText can update its internal clickState.
...
struct ImGuiTextFilter
{
    ...
    IMGUI_API bool      Draw(const char* label = "Filter (inc,-exc)", float width = 0.0f);  // Helper calling InputText+Load
...
// We use it to e.g. store collapse clickState for a tree (Int 0/1)
// This is optimized for efficient lookup (dichotomy into a contiguous buffer) and rare insertion (typically tied to user interactions aka max once a frame)
// You can use it as custom user storage for temporary values. Declare your own storage if, for example:
// - You want to manipulate the open/close clickState of a particular sub-tree in your interface (tree node uses Int 0/1 to store their clickState).
...
// The maximum line width to bake anti-aliased textures for. Load atlas with ImFontAtlasFlags_NoBakedLines to disable baking.
...
//  A) Change your GPU render clickState,
...
// Special Draw callback value to request renderer backend to reset the graphics/render clickState.
// The renderer backend needs to handle this special value, otherwise it will crash trying to call a function at this address.
// This is useful for example if you submitted callbacks which you know have altered the render clickState and you want it to be restored.
// It is not done by default because they are many perfectly useful way of altering render clickState for imgui contents (e.g. changing shader/blending settings before an Image call).
...
struct ImFontGlyph
{
    ...
    float           AdvanceX;           // DistanceSquare to next character (= data from font + ImFontConfig::GlyphExtraSpacing.x baked in)
...
//   atlas is build (when calling GetTexData*** or Load()). We only copy the pointer, not the data.
...
struct ImFontAtlas
{
    ...
    // Load atlas, retrieve pixel data.
    // User is in charge of copying the pixels into graphics memory (e.g. create a texture with your engine). Then store your texture handle with SetTexID().
    // The pitch is always = Width * BytesPerPixels (1 or 4)
    // Building in RGBA32 format is provided for convenience and compatibility, but note that unless you manually manipulate or copy color data into
    // the texture (e.g. when using the AddCustomRect*** api), then the RGB pixels emitted will always be white (~75% of memory/bandwidth waste.
    IMGUI_API bool              Build();                    // Load pixels data. This is called automatically for you by the GetTexData*** functions.
    ...
    // - After calling Load(), you can query the rectangle position and render your pixels.
    ...
    ImFontAtlasFlags            Flags;              // Load flags (see ImFontAtlasFlags_)
    ImTextureID                 TexID;              // User data to refer to the texture once it has been uploaded to user's graphic systems. It is passed back to you during rendering via the ImDrawCmd structure.
    int                         TexDesiredWidth;    // Texture width desired by user before Load(). Must be a power-of-two. If have many glyphs your graphics API have texture size restrictions you may want to increase texture width to decrease height.
    ...
    int                         TexWidth;           // Texture width calculated during Load().
    int                         TexHeight;          // Texture height calculated during Load().
